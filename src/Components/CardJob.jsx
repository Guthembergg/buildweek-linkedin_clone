...
const JobCard = (props) => {
    ...
    return (
        <Card.Body className="d-flex justify-content-between border-bottom">
            ...
            <h6 className="fw-bolder">{props?.singleJob?.company_name.toUpperCase()}</h6>
            <h6><SlLocationPin className="me-2"/>{props?.singleJob?.candidate_required_location}</h6>
            <h6 className="text-secondary">{props?.singleJob?.job_type.toUpperCase()}</h6>
            <h6 className="text-secondary fst-italic">{moment(props?.singleJob?.publication_date).format("DD MM YY") ===
